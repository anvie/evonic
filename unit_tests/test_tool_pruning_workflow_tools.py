"""
Regression test for the "agent finished the task but was never released from
focus" bug (Aira session aira-5340cfec, 2026-09-23).
...
called. Workflow / control-plane tools are characteristically needed LATE in a
turn -- after the threshold has already been crossed -- or at the very start of
the next one:

  * ``state``        drives the plugin state machines. state('kanban:finish') is
                     the ONLY thing that releases task focus (it clears
                     agent_state.focus and the plugin's _active_tasks entry).
  * ``use_skill``    the only way to (re)load a lazy skill's toolset.
  * ``unload_skill`` its counterpart.

When ``state`` was pruned, the agent could finish the work and mark the task
'done' but could never close the workflow, leaving focus=True forever and
blocking the kanban plugin from triggering the agent for subsequent tasks.

Fix: add 'state', 'use_skill' and 'unload_skill' to _ESSENTIAL_TOOLS so the
keep-condition at _prune_tools() retains them for the whole turn.
"""
...
# The workflow / control-plane tools covered by this regression.
WORKFLOW_TOOLS = ('state', 'use_skill', 'unload_skill')


class TestToolPruningProtectsWorkflowTools(unittest.TestCase):
    """Regression: workflow/control-plane tools must survive _prune_tools()."""
    ...
    def _tools_seen(self, llm):
        return [
            [t['function']['name'] for t in call.kwargs['tools']]
            for call in llm.chat_completion.call_args_list
            if (call.kwargs or {}).get('tools') is not None
        ]

    def _drive_four_iterations(self):
        """4 tool-call rounds push _iteration past the threshold, then a final answer."""
        llm = MagicMock()
    ...
    def test_workflow_tools_survive_pruning_after_threshold(self):
        """state / use_skill / unload_skill must still be offered at iteration >= 3."""
        tool_defs = [
            {'type': 'function', 'function': {'name': 'state'}},
            {'type': 'function', 'function': {'name': 'use_skill'}},
            {'type': 'function', 'function': {'name': 'unload_skill'}},
            {'type': 'function', 'function': {'name': 'bash'}},
            {'type': 'function', 'function': {'name': 'calculator'}},
        ]
        llm = self._drive_four_iterations()
        messages = [{'role': 'system', 'content': 'sys'}, {'role': 'user', 'content': 'go'}]
        result, _, _ = self._run_tool_loop(llm, messages, 'sess-prune-workflow', tool_defs)
        self.assertIn('Final answer', str(result))

        tools_seen = self._tools_seen(llm)
        ...
        for name in WORKFLOW_TOOLS:
            self.assertIn(
                name, last_tools,
                '%s (workflow tool) was pruned at iteration >= threshold: %s'
                % (name, last_tools))
        # Non-essential, zero-call tool SHOULD still be pruned -- token optimization retained.
        self.assertNotIn('calculator', last_tools)

    def test_pruned_workflow_tool_toolset_is_not_shrunk_when_idle(self):
        """Sanity: pruning still reports/behaves correctly when nothing but essentials remain."""
        tool_defs = [
            {'type': 'function', 'function': {'name': 'state'}},
            {'type': 'function', 'function': {'name': 'bash'}},
        ]
        llm = self._drive_four_iterations()
        messages = [{'role': 'system', 'content': 'sys'}, {'role': 'user', 'content': 'go'}]
        result, _, _ = self._run_tool_loop(llm, messages, 'sess-prune-workflow-min', tool_defs)
        self.assertIn('Final answer', str(result))
        last_tools = self._tools_seen(llm)[-1]
        self.assertIn('state', last_tools)
        self.assertIn('bash', last_tools)
