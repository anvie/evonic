    ...
    if agent.get('artifacts_enabled', True):
        if agent.get('sandbox_enabled'):
            ...
            artifacts_note = (
                ...
                "NOTE: the `<img>` embed only renders in the web UI. On external chat channels "
                "(WhatsApp, Telegram, Discord, etc.) HTML `<img>` tags and Markdown image "
                "embeds are NOT rendered - always deliver images/files with the `send_file` tool there.\n\n"
