...
- **File placement rule**: NEVER create files directly in the project root. Throwaway/one-off scripts, scratch work, analysis, and temporary output MUST go in your dedicated scratchpad, provided as the `$SCRATCH` env var (a per-agent directory under `/tmp/evonic-temp/`). Only durable, intentional project scripts go in the `scripts/` directory (migration-related scripts in `scripts/migrations/`). Writing a new scratch-like file to the project root will be refused, so route it to `$SCRATCH` or `scripts/` from the start.
